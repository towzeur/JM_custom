...
)
{
  ...
  if (T->stdRange) // clipping not needed
  {
    for (i = 0; i < count; ++i)
    {
      ...
      bp += rgb_stride;
      // convert to YUV
      *yp = (uint16)((T->yr * T->r + T->yg * T->g + T->yb * T->b + T->offy) >> INTEGER_SHIFT);
      yp += yuv_stride;
      *up = (uint16)((T->ur * T->r + T->ug * T->g + T->ub * T->b + T->offuv) >> INTEGER_SHIFT);
      up += yuv_stride;
      *vp = (uint16)((T->vr * T->r + T->vg * T->g + T->vb * T->b + T->offuv) >> INTEGER_SHIFT);
      vp += yuv_stride;
    }
  }
...
)
{
  ...
  for (y = 0; y < yres; y++)
  {
    for (x = 0; x < 8; x += 2)
    {
      ...
      n5 = (x >= 5) ? 5 : x;
      n7 = (x >= 7) ? 7 : x;
      result = (2048 * *(src) + 1228 * (*(src - n1 * srcZres) + *(src + srcZres)) - 262 * (*(src - n3 * srcZres) + *(src + 3 * srcZres)) + 47 * (*(src - n5 * srcZres) + *(src + 5 * srcZres)) + 11 * (*(src - n7 * srcZres) + *(src + 7 * srcZres)) + 2048) / 4096;
      if (result < 0)
        result = 0;
    ...
    for (x = 8; x < limit; x += 2)
    {
      result = (2048 * *(src) + 1228 * (*(src - srcZres) + *(src + srcZres)) - 262 * (*(src - 3 * srcZres) + *(src + 3 * srcZres)) + 47 * (*(src - 5 * srcZres) + *(src + 5 * srcZres)) + 11 * (*(src - 7 * srcZres) + *(src + 7 * srcZres)) + 2048) / 4096;
      if (result < 0)
        result = 0;
      else if (result > pixMax)
        result = pixMax;
    ...
    for (; x < limit; x += 2)
    {
      ...
      result = (2048 * *(src) + 1228 * (*(src - srcZres) + *(src + n1 * srcZres)) - 262 * (*(src - 3 * srcZres) + *(src + n3 * srcZres)) + 47 * (*(src - 5 * srcZres) + *(src + n5 * srcZres)) + 11 * (*(src - 7 * srcZres) + *(src + n7 * srcZres)) + 2048) / 4096;
      if (result < 0)
        result = 0;
      else if (result > pixMax)
        result = pixMax;
...
)
{
  ...
  for (y = 0; y < 6; y += 2)
  {
    ...
    for (x = 0; x < xres; x++)
    {
      result = (225 * (*(src) + *(src + srcRowCount)) + 69 * (*(src - n1 * srcRowCount) + *(src + 2 * srcRowCount)) - 30 * (*(src - n2 * srcRowCount) + *(src + 3 * srcRowCount)) - 16 * (*(src - n3 * srcRowCount) + *(src + 4 * srcRowCount)) + 6 * (*(src - n4 * srcRowCount) + *(src + 5 * srcRowCount)) + 2 * (*(src - y * srcRowCount) + *(src + 6 * srcRowCount)) + 256) / 512;
      if (result < 0)
        result = 0;
      else if (result > pixMax)
        result = pixMax;
  ...
  for (y = 6; y < limit; y += 2)
  {
    for (x = 0; x < xres; x++)
    {
      result = (225 * (*(src) + *(src + srcRowCount)) + 69 * (*(src - srcRowCount) + *(src + 2 * srcRowCount)) - 30 * (*(src - 2 * srcRowCount) + *(src + 3 * srcRowCount)) - 16 * (*(src - 3 * srcRowCount) + *(src + 4 * srcRowCount)) + 6 * (*(src - 4 * srcRowCount) + *(src + 5 * srcRowCount)) + 2 * (*(src - 5 * srcRowCount) + *(src + 6 * srcRowCount)) + 256) / 512;
      if (result < 0)
        result = 0;
      else if (result > pixMax)
        result = pixMax;
  ...
  for (; y < limit; y += 2)
  {
    ...
    for (x = 0; x < xres; x++)
    {
      result = (225 * (*(src) + *(src + n1 * srcRowCount)) + 69 * (*(src - srcRowCount) + *(src + n2 * srcRowCount)) - 30 * (*(src - 2 * srcRowCount) + *(src + n3 * srcRowCount)) - 16 * (*(src - 3 * srcRowCount) + *(src + n4 * srcRowCount)) + 6 * (*(src - 4 * srcRowCount) + *(src + n5 * srcRowCount)) + 2 * (*(src - 5 * srcRowCount) + *(src + n6 * srcRowCount)) + 256) / 512;
      if (result < 0)
        result = 0;
      else if (result > pixMax)
        result = pixMax;
