...
#if (MVC_EXTENSION_ENABLE)
                       || (identify_profile(p_Inp) == MULTIVIEW_HIGH) || (identify_profile(p_Inp) == STEREO_HIGH)
#endif
  );
  // *************************************************************************
  ...
  if (pps->num_slice_groups_minus1 > 0)
  {
    ...
    case 6:
      ...
      pps->pic_size_in_map_units_minus1 =
          (((p_Inp->output.height[0] + p_Vid->auto_crop_bottom) / MB_BLOCK_SIZE) / (2 - sps->frame_mbs_only_flag)) * ((p_Inp->output.width[0] + p_Vid->auto_crop_right) / MB_BLOCK_SIZE) - 1;
