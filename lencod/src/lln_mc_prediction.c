    ...
    { /* other positions */
      ...
      { /* No vertical interpolation */
        for (j = 0; j < block_size_y; j++)
        {
          ...
          {
            result = (*(p0++) + *(p5++)) * COEF[0] + (*(p1++) + *(p4++)) * COEF[1] + (*(p2++) + *(p3++)) * COEF[2];
      ...
      { /* Vertical & horizontal interpolation */
        ...
        for (j = 0; j < block_size_y + 5; j++)
        {
          ...
          for (i = 0; i < block_size_x; i++)
          {
            *(tmp_line++) = (*(p0++) + *(p5++)) * COEF[0] + (*(p1++) + *(p4++)) * COEF[1] + (*(p2++) + *(p3++)) * COEF[2];
          }
        }
        ...
        for (j = 0; j < block_size_y; j++)
        {
          ...
          for (i = 0; i < block_size_x; i++)
          {
            result = (*x0++ + *x5++) * COEF[0] + (*x1++ + *x4++) * COEF[1] + (*x2++ + *x3++) * COEF[2];
      ...
      { /* Horizontal & vertical interpolation */
        ...
        for (j = 0; j < block_size_y; j++)
        {
          ...
          for (i = 0; i < block_size_x + 5; i++)
          {
            ...
            result = (p0[pres_x] + p5[pres_x]) * COEF[0] + (p1[pres_x] + p4[pres_x]) * COEF[1] + (p2[pres_x] + p3[pres_x]) * COEF[2];
            tmp_res[j][i] = result;
          }
        }
