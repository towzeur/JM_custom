  ...
  for (block_y = 0; block_y < 4; block_y++)
  {
    ...
    for (block_x = 0; block_x < 4; block_x++)
    {
      ...
      else if (!p_Vid->active_sps->frame_mbs_only_flag && !currSlice->structure && !currSlice->listX[LIST_1][0]->coded_frame)
      {
        ...
        {
          colocated = p_Vid->active_sps->direct_8x8_inference_flag ? list1[0]->top_field->mv_info[RSD(opic_block_y) >> 1][RSD(opic_block_x)] : list1[0]->top_field->mv_info[(opic_block_y) >> 1][opic_block_x];
        }
        else
        {
          colocated = p_Vid->active_sps->direct_8x8_inference_flag ? list1[0]->bottom_field->mv_info[RSD(opic_block_y) >> 1][RSD(opic_block_x)] : list1[0]->bottom_field->mv_info[(opic_block_y) >> 1][opic_block_x];
        }
      }
      else if (!p_Vid->active_sps->frame_mbs_only_flag && currSlice->structure && list1[0]->coded_frame)
      {
        ...
        if (currSlice->structure != list1[0]->structure)
        {
          ...
          {
            colocated = p_Vid->active_sps->direct_8x8_inference_flag ? list1[0]->frame->top_field->mv_info[RSD(opic_block_y)][RSD(opic_block_x)] : list1[0]->frame->top_field->mv_info[opic_block_y][opic_block_x];
          }
          else
          {
            colocated = p_Vid->active_sps->direct_8x8_inference_flag ? list1[0]->frame->bottom_field->mv_info[RSD(opic_block_y)][RSD(opic_block_x)] : list1[0]->frame->bottom_field->mv_info[opic_block_y][opic_block_x];
          }
        }
...
int get_colocated_info_4x4(Macroblock *currMB, StorablePicture *list1, int i, int j)
{
  ...
  else
  {
    ...
    int moving = !((((fs->ref_idx[LIST_0] == 0) && (iabs(fs->mv[LIST_0].mv_x) >> 1 == 0) && (iabs(fs->mv[LIST_0].mv_y) >> 1 == 0))) || ((fs->ref_idx[LIST_0] == -1) && (fs->ref_idx[LIST_1] == 0) && (iabs(fs->mv[LIST_1].mv_x) >> 1 == 0) && (iabs(fs->mv[LIST_1].mv_y) >> 1 == 0)));
...
void Get_Direct_MV_Spatial_MBAFF(Macroblock *currMB)
{
  ...
  for (block_y = 0; block_y < 4; block_y++)
  {
    ...
    for (block_x = 0; block_x < 4; block_x++)
    {
      ...
      // Test Level Limits if satisfied.
      if ((out_of_bounds_mvs(p_Vid, &all_mvs[LIST_0][l0_refX < 0 ? 0 : l0_refX][0][block_y][block_x]) || out_of_bounds_mvs(p_Vid, &all_mvs[LIST_1][l1_refX < 0 ? 0 : l1_refX][0][block_y][block_x])))
      {
        direct_ref_idx[LIST_0] = -1;
        direct_ref_idx[LIST_1] = -1;
        currSlice->direct_pdir[pic_block_y][pic_block_x] = -1;
