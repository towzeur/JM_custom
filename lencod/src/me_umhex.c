...
int UMHEX_get_mem(VideoParameters *p_Vid, InputParameters *p_Inp)
{
  ...
  if (NULL == (p_UMHex->flag_intra = calloc((p_Vid->width >> 4) + 1, sizeof(byte))))
    ...
    else
    {
      rFrameL = block_a.available
                    ? (p_Vid->mb_data[block_a.mb_addr].mb_field
                           ...
                           : mv_info[block_a.pos_y][block_a.pos_x].ref_idx[list])
                    : -1;
      rFrameU = block_b.available ? p_Vid->mb_data[block_b.mb_addr].mb_field ? mv_info[block_b.pos_y][block_b.pos_x].ref_idx[list] >> 1 : mv_info[block_b.pos_y][block_b.pos_x].ref_idx[list] : -1;
      rFrameUR = block_c.available ? p_Vid->mb_data[block_c.mb_addr].mb_field ? mv_info[block_c.pos_y][block_c.pos_x].ref_idx[list] >> 1 : mv_info[block_c.pos_y][block_c.pos_x].ref_idx[list] : -1;
    }
  }
  ...
  for (hv = 0; hv < 2; hv++)
  {
    ...
    if (p_Inp->UMHexDSR)
    {
      ...
      else
      {
        ...
        if (dsr_mv_sum == 0)
          dsr_small_search_range = (p_Inp->search_range[p_Vid->view_id] + 4) >> 3;
        else if (dsr_mv_sum > 3)
          dsr_small_search_range = (p_Inp->search_range[p_Vid->view_id] + 2) >> 2;
        else
          dsr_small_search_range = (3 * p_Inp->search_range[p_Vid->view_id] + 8) >> 4;
