...
short hme_EPZSSpatialPredictors(EPZSParameters *p_EPZS,
                                MEBlock *mv_block,
                                MotionVector **pic_mv)
{
  PixelPos *block = mv_block->block;
...
void hme_EPZSWindowPredictors(MotionVector *mv, EPZSStructure *predictor, int *prednum, EPZSStructure *windowPred, int level)
{
  int pos;
  ...
  *EPZSPoint = p_EPZS->BlkCount;
  ...
  if (min_mcost > stopCriterion)
  {
    ...
    */
    ...
    conditionEPZS = ((min_mcost > 3 * stopCriterion) && ((ref < 2 && blocktype < 4) || (ref < 1 && blocktype == 4)) && (p_Inp->EPZSFixed > 1 || (p_Inp->EPZSFixed && currSlice->slice_type == P_SLICE)) && (mv_block->hme_level == p_Vid->pHMEInfo->iPyramidLevels - 1));
    ...
    */
    ...
    if (min_mcost > stopCriterion)
    {
      ...
      for (;;)
      {
        ...
        conditionEPZS = (checkMedian == TRUE) && (ref == 0 || (ref > 0 && min_mcost < 2 * neighbor_dist)) && (min_mcost > ((3 * stopCriterion) >> 1)) && (p_Inp->EPZSDual > 0);
