...
int update_video_stats(VideoParameters *p_Vid)
{
  ...
  if (p_Inp->RCEnable)
  {
    ...
    {
      bits = (int)(p_Vid->p_Stats->bit_ctr - p_Vid->p_Stats->bit_ctr_n) + (int)(p_Vid->p_Stats->bit_ctr_filler_data - p_Vid->p_Stats->bit_ctr_filler_data_n);
    }
    else if (p_Inp->RCUpdateMode <= MAX_RC_MODE)
    {
      bits = (int)(p_Vid->p_Stats->bit_ctr - (p_Vid->p_rc_quad->Pprev_bits)) + (int)(p_Vid->p_Stats->bit_ctr_filler_data - p_Vid->p_Stats->bit_ctr_filler_data_n); // used for rate control update
...
int encode_one_frame(VideoParameters *p_Vid, InputParameters *p_Inp)
{
  ...
  */
  ...
  if (!p_Vid->redundant_coding)
  {
    p_Vid->Bit_Buffer[p_Vid->total_frame_buffer++] = (long)(p_Vid->p_Stats->bit_ctr - p_Vid->p_Stats->bit_ctr_n) + (long)(p_Vid->p_Stats->bit_ctr_filler_data - p_Vid->p_Stats->bit_ctr_filler_data_n);
  }
#endif
  ...
  if (p_Vid->num_of_layers == 2)
  {
    ...
    else if (p_Vid->fld_flag && p_Vid->view_id == 1)
    {
      printf("%05d(%3s)  %1d  %8d %3d  %1d %2d %4d %7.3f %7.3f %7.3f %9d %7d    %3s %5d %1d %2d %2d  %d   %d\n",
             p_Vid->prev_cs.frm_no_in_file, p_Vid->prev_cs.pic_type, p_Vid->prev_cs.view_id,
             (int)(p_Vid->p_Stats->bit_ctr_v[0] - p_Vid->p_Stats->bit_ctr_n_v[0]) + (int)(p_Vid->p_Stats->bit_ctr_filler_data_v[0] - p_Vid->p_Stats->bit_ctr_filler_data_n_v[0]) + p_Vid->p_Stats->bit_ctr_parametersets_n_v[0],
      ...
      printf("%05d(%3s)  %1d  %8d %3d  %1d %2d %4d %7.3f %7.3f %7.3f %9d %7d    %3s %5d %1d %2d %2d  %d   %d\n",
             p_Vid->frm_no_in_file, pic_type, p_Vid->view_id,
             (int)(p_Vid->p_Stats->bit_ctr_v[1] - p_Vid->p_Stats->bit_ctr_n_v[1]) + (int)(p_Vid->p_Stats->bit_ctr_filler_data_v[1] - p_Vid->p_Stats->bit_ctr_filler_data_n_v[1]) + p_Vid->p_Stats->bit_ctr_parametersets_n_v[1],
...
static void ReportFirstframe(VideoParameters *p_Vid, int64 tmp_time)
{
  ...
  int cur_bits = (int)(stats->bit_ctr - stats->bit_ctr_n) + (int)(stats->bit_ctr_filler_data - stats->bit_ctr_filler_data_n);
