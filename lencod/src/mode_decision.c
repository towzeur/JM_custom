...
void end_encode_one_macroblock(Macroblock *currMB)
{
  ...
  if ((currSlice->mb_aff_frame_flag) && (currMB->mbAddrX & 0x01) && (currMB->mb_type ? 0 : (((currSlice->slice_type == B_SLICE)) ? !currMB->cbp : 1))                  // bottom is skip
      && ((currMB->PrevMB)->mb_type ? 0 : (((currSlice->slice_type == B_SLICE)) ? !(currMB->PrevMB)->cbp : 1)) && !(field_flag_inference(currMB) == currMB->mb_field)) // top is skip
  {
                          ...
                          short *inter_skip)
{
  ...
  currMB->luma_transform_size_8x8_flag = (byte)(p_Inp->Transform8x8Mode == 2
                                                    ? (mode >= 1 && mode <= 3) || (mode == 0 && bslice && p_Vid->active_sps->direct_8x8_inference_flag) || ((mode == P8x8) && (enc_mb->valid[4]))
                                                    : 0);
  ...
  if ((p_Inp->FastCrIntraDecision) || (currMB->c_ipred_mode == DC_PRED_8 || (is_intra(currMB))))
  {
    ...
    if ((bslice && mode == 0 && (*inter_skip == 0) && enc_mb->valid[mode] && currMB->cbp && (currMB->cbp & 15) != 15 && !p_Inp->nobskip && !(currMB->qp_scaled[0] == 0 && p_Vid->active_sps->lossless_qpprime_flag == 1)))
    {
