...
void frame_picture_mp_b_slice(VideoParameters *p_Vid, InputParameters *p_Inp)
{
  ...
  {
    // frame QP pass
    p_Vid->active_pps = best_method == EXP_WP ? p_Vid->PicParSet[1] : best_method == IMP_WP ? p_Vid->PicParSet[2]
                                                                                            : p_Vid->PicParSet[0];
    // p_Vid->qp = (p_Vid->nal_reference_idc==0 ? rd_qp + 1 : rd_qp - 1);
  ...
  if (p_Inp->RDPictureDirectMode)
  {
    ...
    p_Vid->active_pps = apply_wp == IMP_WP ? p_Vid->PicParSet[1] : apply_wp == EXP_WP ? p_Vid->PicParSet[2]
                                                                                      : p_Vid->PicParSet[0];
