...
void dpb_split_field(VideoParameters *p_Vid, FrameStore *fs)
{
  ...
  if (!p_Vid->active_sps->frame_mbs_only_flag)
  {
    ...
    fs->top_field->used_for_reference = fs->bottom_field->used_for_reference = frame->used_for_reference;
    fs->top_field->is_long_term = fs->bottom_field->is_long_term = frame->is_long_term;
    fs->long_term_frame_idx = fs->top_field->long_term_frame_idx = fs->bottom_field->long_term_frame_idx = frame->long_term_frame_idx;

    fs->top_field->coded_frame = fs->bottom_field->coded_frame = 1;
    fs->top_field->mb_aff_frame_flag = fs->bottom_field->mb_aff_frame_flag = frame->mb_aff_frame_flag;
