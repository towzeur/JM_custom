...
int CheckReliabilityOfRef(Macroblock *currMB, int block, int list_idx, int ref, int mode)
{
  ...
  int by0 = (mode >= 4 ? 2 * (block >> 1) : mode == 2 ? 2 * block
                                                      : 0);
  int by1 = by0 + (mode >= 4 || mode == 2 ? 2 : 4);
  int bx0 = (mode >= 4 ? 2 * (block & 0x01) : mode == 3 ? 2 * block
                                                        : 0);
...
void copy_rdopt_data(Macroblock *currMB)
{
  ...
  for (j = 0; j < BLOCK_MULTIPLE; j++)
  {
    ...
    for (i = 0; i < BLOCK_MULTIPLE; i++)
    {
      ...
      motion[block_y][currMB->block_x + i].ref_pic[LIST_0] = rdopt->refar[LIST_0][j][i] < 0 ? NULL : currSlice->listX[LIST_0 + list_offset][(short)rdopt->refar[LIST_0][j][i]];
    }
  }
  ...
  if (currSlice->slice_type == B_SLICE)
  {
    for (j = 0; j < BLOCK_MULTIPLE; j++)
    {
      ...
      for (i = 0; i < BLOCK_MULTIPLE; i++)
      {
        ...
        motion[block_y][currMB->block_x + i].ref_pic[LIST_1] = rdopt->refar[LIST_1][j][i] < 0 ? NULL : currSlice->listX[LIST_1 + list_offset][(short)rdopt->refar[LIST_1][j][i]];
      }
    }
  }
