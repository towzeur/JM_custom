  ...
  else if (p_Inp->intra_period)
  {
    ...
    else // display order (insertion)
    {
      ...
      for (idx = (p_seq_struct->num_intra_gops - 1); idx >= 0; idx--)
      {
        ...
        case 2:
          // see comment above
          if (((curr_frame + offset) - imax(p_seq_struct->last_idr_disp, p_seq_struct->last_intra_disp)) % p_Inp->intra_period == 0 && rand_acc_test_range(curr_frame - imax(p_seq_struct->last_idr_disp, p_seq_struct->last_intra_disp), offset, p_Inp->idr_period))
          {
            is_intra = 1 + idx;
          }
          break;
  ...
  for (curr_prd = (p_seq_struct->num_prds - 1); curr_prd >= 0; curr_prd--)
  {
    ...
    case 1: // even/odd hierarchy
      ...
      for (idx = 1; idx < length2; idx++)
      {
        ...
        p_dst->layer = 1;
        p_dst->slice_qp_off = p_dst->slice_type == B_SLICE ? (p_Inp->HierarchyLevelQPEnable ? -1 : p_Inp->qpBRSOffset) : (p_Inp->HierarchyLevelQPEnable ? 1 : 0); // 1
        p_dst->random_access = 0;
        p_dst->temporal_layer = 0;
      }
      ...
      if (length > 3)
      {
        ...
        for (layer = 1, order = 1, sum = 0, frm = 1; sum < (length - 1); frm = (frm << 1), sum += frm, layer++)
        {
          ...
          for (idx = 0; idx < frm; idx++, order++)
          {
            ...
            p_dst->layer = layer;
            p_dst->slice_qp_off = p_dst->slice_type == B_SLICE ? (p_Inp->HierarchyLevelQPEnable ? (-GOPlevels + layer) : (p_dst->nal_ref_idc != NALU_PRIORITY_DISPOSABLE ? p_Inp->qpBRSOffset : 0)) : (p_Inp->HierarchyLevelQPEnable ? layer : 0); // layer
            p_dst->random_access = 0;
            p_dst->temporal_layer = 0;
          }
        }
...
void populate_reg_pic(InputParameters *p_Inp, PicStructure *p_pic, FrameUnitStruct *p_frm_struct, int num_slices, int is_bot_fld)
{
  ...
  int deblock = p_Inp->DFSendParameters ? (!(p_Inp->DFDisableIdc[p_frm_struct->nal_ref_idc][p_frm_struct->type])) : 1;
  int weighted_pred = (slice_type == P_SLICE) ? p_Inp->WeightedPrediction : ((slice_type == B_SLICE) ? p_Inp->WeightedBiprediction : 0);
