...
#define isBlock(object_list, currMBNum, comp, regMode) \
    (isSplitted(object_list, currMBNum) ? ((object_list + ((currMBNum) << 2) + (comp))->regionMode == REGMODE_##regMode##_8x8) : ((object_list + ((currMBNum) << 2))->regionMode == REGMODE_##regMode))

/* this can be used as getParam(...,mv) or getParam(...,xMin) or getParam(...,yMin) */
#define getParam(object_list, currMBNum, comp, param) \
    (isSplitted(object_list, currMBNum) ? ((object_list + ((currMBNum) << 2) + (comp))->param) : ((object_list + ((currMBNum) << 2))->param))
