          ...
          else            // for everything else, if no coefs, but vector difference >= 1 set Strength=1
          {
            ...
            if (((ref_p0 == ref_q0) && (ref_p1 == ref_q1)) || ((ref_p0 == ref_q1) && (ref_p1 == ref_q0)))
            {
              ...
              if (ref_p0 != ref_p1)
              {
                ...
                if (ref_p0 == ref_q0)
                {
                  StrValue = (byte)(compare_mvs(&mv_info_p->mv[LIST_0], &mv_info_q->mv[LIST_0], mvlimit) ||
                                    compare_mvs(&mv_info_p->mv[LIST_1], &mv_info_q->mv[LIST_1], mvlimit));
                }
                else
                {
                  StrValue = (byte)(compare_mvs(&mv_info_p->mv[LIST_0], &mv_info_q->mv[LIST_1], mvlimit) ||
              ...
              get_mb_block_pos_mbaff(PicPos, MbQ->mbAddrX, &mb_x, &mb_y);
              {
                ...
                if (((ref_p0 == ref_q0) && (ref_p1 == ref_q1)) || ((ref_p0 == ref_q1) && (ref_p1 == ref_q0)))
                {
                  ...
                  if (ref_p0 != ref_p1)
                  {
                    ...
                    if (ref_p0 == ref_q0)
                    {
                      Strength[idx] = (byte)(compare_mvs(&mv_info_p->mv[LIST_0], &mv_info_q->mv[LIST_0], mvlimit) ||
                                             compare_mvs(&mv_info_p->mv[LIST_1], &mv_info_q->mv[LIST_1], mvlimit));
                    }
                    else
                    {
                      Strength[idx] = (byte)(compare_mvs(&mv_info_p->mv[LIST_0], &mv_info_q->mv[LIST_1], mvlimit) ||
                                             compare_mvs(&mv_info_p->mv[LIST_1], &mv_info_q->mv[LIST_0], mvlimit));
                    }
                  }
                  else
