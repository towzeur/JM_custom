...
void activate_sps(VideoParameters *p_Vid, seq_parameter_set_rbsp_t *sps)
{
  ...
  if (p_Vid->active_sps != sps)
  {
    ...
    else if (p_Vid->last_profile_idc != p_Vid->active_sps->profile_idc && (is_MVC_profile(p_Vid->last_profile_idc) || is_MVC_profile(p_Vid->active_sps->profile_idc)) && (!p_Vid->p_Dpb_layer[1]->init_done))
    {
