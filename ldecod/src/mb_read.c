...
static void read_ipred_8x8_modes_mbaff(Macroblock *currMB)
{
  ...
  for (b8 = 0; b8 < 4; ++b8) // loop 8x8 blocks
  {
    ...
    mostProbableIntraPredMode = (upIntraPredMode < 0 || leftIntraPredMode < 0) ? DC_PRED : upIntraPredMode < leftIntraPredMode ? upIntraPredMode
                                                                                                                               : leftIntraPredMode;
...
void skip_macroblock(Macroblock *currMB)
{
  ...
  zeroMotionLeft = !mb[0].available ? 1 : a_ref_idx == 0 && a_mv->mv_x == 0 && a_mv_y == 0 ? 1
                                                                                           : 0;
  zeroMotionAbove = !mb[1].available ? 1 : b_ref_idx == 0 && b_mv->mv_x == 0 && b_mv_y == 0 ? 1
                                                                                            : 0;
